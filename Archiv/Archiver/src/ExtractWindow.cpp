    ...
    else
    {
        ...
        if (this->ui->RLERadioButton->isChecked())
        {
            for(auto it:filesToDecompress)
            {
                ...
                QString rleDecompressFileName =  tempDir.append(it.split("/").back().split(".").front().append("_RleDecompress.txt").prepend("/"));
                compressorRle.decompressRLE(it, rleDecompressFileName);
            }
        } else {
            for(auto it:filesToDecompress)
            {
                QString tempDir = direct;
                QString huffmanDecompressFileName =  tempDir.append(it.split("/").back().split(".").front().append("_Huffmandecompress.txt").prepend("/"));
